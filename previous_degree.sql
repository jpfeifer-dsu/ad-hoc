...
select pidm,
      ...
      from (with cte_prev_dgr as (select shrdgmr_pidm, max(shrdgmr_seq_no) as shrdgmr_seq_no
                                  from shrdgmr
                                  where shrdgmr_seq_no > 1
                                  group by shrdgmr_pidm),
                 -- Grabs Highest Degree earned from a previous institution
                 cte_prev_dgr2 as (select sordegr_pidm as pidm,
                 ...
                 cte_prev_dgr3 as (select pidm, stvdegc_code, deg_rank
                                   from cte_prev_dgr2,
                                        saturn.sordegr,
                                        saturn.stvdegc,
                                        spriden
                                   where pidm = sordegr_pidm
                                     ...
                                     and spriden_change_ind is null
                                     and sordegr_degc_date <= to_date('01-JUN-2019')),
                 cte_prev_deg4 as (select shrtram_pidm,
                                          shrtram_degc_code,
                                          case
                                             when stvdegc_dlev_code = 'DR' then '5'
                                             when stvdegc_dlev_code = 'MA' then '4'
            ...
            where s2.shrdgmr_seq_no <> s1.shrdgmr_seq_no and pidm = '16924')
order by 1;
