WITH cte_HSCE_2019 AS (
    -- USHE YEAR 2020
    SELECT DISTINCT 'Fall 2019'                     AS Term_Desc,
                    ...
                    CASE
                        WHEN shrtrce_grde_code NOT LIKE '%D%'
                            AND shrtrce_grde_code NOT LIKE '%F%'
                            AND shrtrce_grde_code NOT LIKE '%W%'
                            AND shrtrce_grde_code NOT LIKE '%C-'
                            THEN 1
                        ELSE 0
                        END                         AS Passed_Count,
                    CASE
                        WHEN shrtrce_grde_code NOT LIKE '%D%'
                            ...
                            AND shrtrce_grde_code NOT LIKE '%W%'
                            AND shrtrce_grde_code NOT LIKE '%C-'
                            THEN shrtrce_credit_hours
                        ELSE 0 END                  AS Earn_Cr
    FROM spriden
    ...
    SELECT DISTINCT 'Fall 2019'                                        AS Term_Desc,
                    ...
                    CASE
                    WHEN scbcrse_subj_code = 'EL'
                        THEN 'EL ' || to_char(lpad(scbcrse_crse_numb, 4, '0')) || ' - Various'
                    ELSE SCBCRSE_TITLE
                    END                                            AS Course_Title,
                    swvgrde_final_grade,
                CASE
                    WHEN swvgrde_final_grade NOT LIKE '%D%'
                        AND swvgrde_final_grade NOT LIKE '%F%'
                        AND swvgrde_final_grade NOT LIKE '%W%'
                        AND swvgrde_final_grade NOT LIKE '%C-'
                        THEN 1
                    ELSE 0 END                                     AS Grade,
                    CASE
                    WHEN swvgrde_final_grade NOT LIKE '%D%'
